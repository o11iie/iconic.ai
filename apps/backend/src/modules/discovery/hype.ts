 ...
 * present) with release proximity, so upcoming titles trend up as they
 * approach release. This must always be presented in the UI as "Slate's
 * Hype Score" (an opinionated ranking), never as a claim of real-world
 * buzz data Slate does not actually have (e.g. social media volume).
 */
export function computeHypeScore(title: TitleSummary): number {
  const popularity = title.voteAverage ? title.voteAverage / 10 : 0.3; // neutral default when unrated (upcoming titles)
